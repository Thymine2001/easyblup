    ...
    if (length(values$fixed) > 0) {
      for (eff in values$fixed) {
        ...
        if (nzchar(effect_cols)) {
          # Only add "alpha" for cross effects, not for cov
          alpha_str <- if (effect_type == "cross") " alpha" else ""
          param_text <- paste0(param_text,
            "EFFECT\n",
            effect_cols, " ", effect_type, alpha_str, " # ", eff, " fixed effect (", effect_type, ") (trait order: 1, 2, ...)\n"
          )
        } else {
          param_text <- paste0(param_text,
            "EFFECT\n",
